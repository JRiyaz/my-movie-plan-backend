...
public interface MovieShowsRepository extends JpaRepository<MovieShowsEntity, Integer> {
    ...
    // findDistinctByMovieIdAndShowStartAfter
    @Query(value = "SELECT * FROM movie_shows ms WHERE ms.show_start > CURRENT_DATE", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM movie_shows ms WHERE ms.show_start <= CURRENT_DATE AND ms.show_end >= CURRENT_DATE LIMIT :records", nativeQuery = true)
